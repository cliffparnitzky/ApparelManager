...
class tl_apparel_article_variant extends Backend
{
  ...
  public function prepareHeader($arrHeaderFields, DataContainer $dc)
  {
    ...
    $arrHeaderFields[$GLOBALS['TL_LANG']['tl_apparel_article']['productLink'][0]] = (!empty($productLink) ? '<a href="' . $productLink . '" target="_blank">' .  \ApparelManagerHelper::trimLongText($productLink) . '</a>' : '&nbsp;');
    ...
    $arrHeaderFields[$GLOBALS['TL_LANG']['tl_apparel_article']['comment'][0]] = (!empty($comment) ? \ApparelManagerHelper::trimLongText($comment) : '&nbsp;');
...
<div>
  ...
  <table class="tl_apparel_child">
    ...
    <tr><td><span class="tl_label">' . $GLOBALS['TL_LANG']['tl_apparel_article_variant']['comment'][0] . ':</span></td><td>' . (!empty($row['comment']) ? \ApparelManagerHelper::trimLongText($row['comment']) : '&nbsp;') . '</td></tr>
  </table>
</div>' . "\n";
  }
