...
<div class="tl_submit_container">
  ...
  <table class="tl_apparel_child">
    ...
    <tr><td><span class="tl_label">' . $GLOBALS['TL_LANG']['tl_apparel_article']['productLink'][0] . ':</span></td><td>' . (!empty($objApparelArticle->productLink) ? '<a href="' . $objApparelArticle->productLink . '" target="_blank">' .  static::trimLongText($objApparelArticle->productLink) . '</a>' : '&nbsp;') . '</td></tr>
    <tr><td><span class="tl_label">' . $GLOBALS['TL_LANG']['tl_apparel_article']['comment'][0] . ':</span></td><td>' . (!empty($objApparelArticle->comment) ? static::trimLongText($objApparelArticle->comment) : '&nbsp;') . '</td></tr>
  ...
  /**
   * Trim a long text to the specefied length.
   */
  public static function trimLongText($strText, $intLength=70)
  {
    if (version_compare(VERSION . '.' . BUILD, '3.5.1', '>='))
    {
      return trim(\StringUtil::substr($strText, $intLength));
    }
    return trim(\String::substr($strText, $intLength));
  }
}
